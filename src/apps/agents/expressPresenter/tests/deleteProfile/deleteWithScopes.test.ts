...
describe('expressPresenter.deleteProfile with scopes', () => {
  ...
  it('should throw forbidden error when using invalid scope', async () => {
    await deleteProfile().set('Authorization', TEST_INVALID_SCOPE_TOKEN).expect(FORBIDDEN);
  });
  ...
  it('should not error when using valid scopes', async () => {
    await deleteProfile().set('Authorization', TEST_VALID_SCOPE_TOKEN).expect(NO_CONTENT);
  });
});
