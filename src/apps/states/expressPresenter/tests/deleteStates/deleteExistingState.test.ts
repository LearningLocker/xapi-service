...
describe('expressPresenter.deleteStates with existing state', () => {
  ...
  it('should delete when deleting with an mbox', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_MBOX_AGENT) }).expect(StatusCodes.NO_CONTENT);
  ...
  it('should delete when deleting with an mbox_sha1sum', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_MBOXSHA1_AGENT) }).expect(
  ...
  it('should delete when deleting with an openid', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_OPENID_AGENT) }).expect(StatusCodes.NO_CONTENT);
  ...
  it('should delete when deleting with an account', async () => {
    ...
    await deleteStates({ agent: JSON.stringify(TEST_ACCOUNT_AGENT) }).expect(
      StatusCodes.NO_CONTENT,
    );
